...
export async function submitOnboardingForm(formData: FormData) {
  ...
  try {
    if (userType === "resident") {
      ...
      const { error } = await supabase.from("at_risk").insert(data);
      ...
      const { error } = await supabase.from("dispatchers").insert(data);
      ...
      const data = {
        role: formData.get(
          "role",
        ) as Database["public"]["Enums"]["first_responder_role"],
      ...
      const { error } = await supabase.from("first_responders").insert(data);
