...
export const getUser = async (supabase: SupabaseClient<Database>): Promise<User | null> => {
    const { data: { user } } = await supabase.auth.getUser();
    return user;
}

export const getUserRole = async (user: User, supabase: SupabaseClient<Database>): Promise<ResidentData | DispatcherData | FirstResponderData | null> => {
